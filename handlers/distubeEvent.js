...
module.exports = (client) => {
  try {
    client.distube
          ...
          collector.on('collect', async i => {
            ...
            if(i.customId == `10`){let { member } = i;
              ...
              if (channel.id !== newQueue.voiceChannel.id)
                ...
                return i.reply({
                  content: `${client.allEmojis.x} **Lyrics are disabled!**\n> *Lyrics are disabled and won't work for an unknown amount of time* :cry:`,
                  ephemeral: true
                });
              let embeds = [];
      ...
      .on(`finishSong`, (queue, song) => {
        ...
        .setThumbnail(`https://img.youtube.com/vi/${song.id}/mqdefault.jpg`)
        .setFooter(` ${song.user.tag}\n SONG ENDED!`, song.user.displayAvatarURL({
          dynamic: true
        }));
  ...
  function receiveQueueData(newQueue, newTrack) {
    ...
    var embed = new MessageEmbed().setColor(ee.color)
      ...
      .setAuthor(`${newTrack.name}`, `https://cdn.discordapp.com/attachments/856114693739773952/873890345518235648/PicsArt_08-06-06.25.14.jpg`, newTrack.url)
    ...
    let songloop = new MessageButton().setStyle('SUCCESS').setCustomId('6').setEmoji(`🔁`)
    let queueloop = new MessageButton().setStyle('SUCCESS').setCustomId('7').setEmoji(`🔂`)
    ...
    const row = new MessageActionRow().addComponents([stop, rewind, pause, forward]);
    const row2 = new MessageActionRow().addComponents([skip, songloop, queueloop, shuffle]);
    return {
      embeds: [embed],
      components: [row, row2]
    };
